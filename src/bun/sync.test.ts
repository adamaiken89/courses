...
let mockRepoURL = '';
let mockLastSyncedCommit: string | null = null;

mock.module('./storage', () => ({
  getSyncConfig: () => ({
    remoteRepoURL: mockRepoURL,
    lastSyncedCommit: mockLastSyncedCommit,
    lastSyncTime: null,
  }),
  saveSyncConfig: (config: { remoteRepoURL?: string; lastSyncedCommit?: string | null }) => {
    if (config.remoteRepoURL !== undefined) mockRepoURL = config.remoteRepoURL;
    if (config.lastSyncedCommit !== undefined) mockLastSyncedCommit = config.lastSyncedCommit;
  },
  getGeminiKey: () => null,
  setGeminiKey: (_key: string) => {},
}));
...
describe('syncCourses', () => {
  ...
  test('returns up-to-date when same commit', async () => {
    mockRepoURL = 'https://github.com/owner/repo';
    mockLastSyncedCommit = 'abc123';
  ...
  test('returns error on invalid repo URL format', async () => {
    mockRepoURL = 'https://gitlab.com/owner/repo';
    mockSubjectsDir = '/tmp/subjects';
